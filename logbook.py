...
if selected == "Phase 3: Molecular Docking and Dynamics Simulation":
    with st.container():
       ...
       """, language="bash")
        ...
        st.write("**24. run MD simulation of the CPB OctB2R-ligand complex inside the lipid bilayer model via GROMACS for 100 ns first, followed by extending the MD simulation to 500 ns and to 1000 ns**")
        st.write("✔️assign protein as chain A and ligand as chain B, followed by changing the ligand name from UNL to AMZ (three letter code that i set personally for amitraz) after saving the protein-ligand complex (derived from AutoDock Vina result) in PDB format")
        st.code("""
        ...
        st.write("✔️run the MD production simulation for 100 ns first, followed by extending the MD simulation to 500 ns and to 1000 ns")
        st.code("""
        nohup gmx grompp -f step7_production.mdp -c step6.6.gro -t step6.6.cpt -p topol.top -n index.ndx -o md_100ns.tpr > step7.0_tpr.log 2>&1 & # remember to modify the step7_production.mdp file by changing the values of dt (stepsize) and nsteps (total number of steps) to 100ns first before running this command to generate the portable binary run input file.
        nohup gmx mdrun -s md_100ns.tpr -deffnm md_100ns -nb gpu -pme gpu -bonded gpu -update gpu > step7.0_100ns_mdrun.log 2>&1 & # run the 100 ns production simulation with GPU acceleration
        nohup gmx convert-tpr -s md_100ns.tpr -extend 400000 -o md_500ns.tpr > extend_400000_mdrun.log 2>&1 & # extend the simulation from 100 ns → 500 ns
        nohup gmx mdrun -s md_500ns.tpr -deffnm md_500ns -cpi md_100ns.cpt -noappend -nb gpu -pme gpu -bonded gpu -update gpu > step7.0_500ns_mdrun.log 2>&1 & # continue running the simulation up to 500 ns with GPU acceleration (to preserve the atom's velocities, thermostat state, barostat state, random seeds, and trajectory continuity)
        nohup gmx convert-tpr -s md_500ns.tpr -extend 500000 -o md_1000ns.tpr > extend_500000_mdrun.log 2>&1 & # extend the simulation from 500 ns → 1000 ns
        ...
        st.write("✔️create an index group for TM helices and ligand only, followed by extracting the topology information and MD simulation trajectory of TM helices and ligand only. Expect to observe lower RMSD values since loops no longer dominate the motion, termini no longer inflate RMSD, and only the GPCR core is measured")
        st.code("""
        gmx make_ndx -f protein_ligand_complex.tpr -n index.ndx -o tm_helices_ligand.ndx # create the index for both TM helices of the protein and its ligand only. Type, r 31-54 | r 66-87 | r 104-125 | r 146-167 | r 188 | r 190 | r 194 | r 195-215 | r 279-298 | r 311-334 | 13, (13 indicates the AMZ ligand) and press "Enter", followed by typing, name 19 TM_helices_ligand, to rename the newly created index group as TM_helices_ligand and position the newly created index group as 19. Type "q" to quit
        gmx convert-tpr -s protein_ligand_complex.tpr -n tm_helices_ligand.ndx -o tm_helices_ligand.tpr # Select 19 (TM_helices_ligand) to extract TM_helices_ligand-only topology
        gmx trjconv -s protein_ligand_complex.tpr -f protein_ligand_complex_fit.xtc -n tm_helices_ligand.ndx -o tm_helices_ligand_fit.xtc # Select 19 (TM_helices_ligand) to extract TM_helices_ligand-only trajectory
        """, language="bash")
        ...
        st.write("✔️calculate the TM-helices only's and protein's backbone RMSD to investigate how much did the protein structure deviate from the reference structure over time to determine structural stability, equilibration quality, large conformational changes, and simulation convergence. To answer questions like does the insect receptor structure maintain its structure? does the binding pocket collapse? does the transmembrane region stay stable?")
        st.code("""
        gmx rms -s tm_helices_ligand.tpr -f tm_helices_ligand_fit.xtc -tu ns -o tm_helices_ligand_rmsd.xvg # Select group for least squares fit: Backbone and Select group for RMSD calculation: Backbone
