...
if selected == "Phase 1: Sequence-Based Analysis":
    with st.container():
        ...
        st.code("""
        nohup cd-hit-est -i /media/raid/Wee/WeeYeZhi/output/trinity_results/trinity_assembly_done_by_LKM_and_INBIOSIS/trinity_assembly_done_by_LKM_and_INBIOSIS.Trinity.fasta -o /media/raid/Wee/WeeYeZhi/output/cd_hit_est_results/cd_hit_est_after_trimmomatic -c 0.95 -n 5 -T 4 > cd_hit_est_after_trimmomatic_output.log 2>&1 & # Do not create the directory, cd_hit_est_after_trimmomatic before running this command, run this command in the working directory, /media/raid/Wee/WeeYeZhi/output/cd_hit_est_results, as the command will later automatically create the output directory for you, otherwise you will encounter file opening error, specify fewer number of threads (use -T 4 in this case) if possible to avoid encountering OutofMemory error
...
if selected == "Phase 3: Molecular Docking and Dynamics Simulation":
    with st.container():
       ...
       """, language="bash")
        ...
        st.markdown("[Watch YouTube video to learn all the different file formats, .gro, .top, .itp, .ndx, .mdp, .tpr, .log, .edr, .trr, .xtc, and .cpt files](https://www.youtube.com/watch?v=cUfzp8Fubao&t=196s)")
        ...
        st.code("""
        nohup gmx grompp -f step6.0_minimization.mdp -c step5_input.gro -r step5_input.gro -p topol.top -o em.tpr > em_tpr.log 2>&1 & # navigate to the working directory, /media/raid/Wee/WeeYeZhi/output/charmmgui_results/charmm-gui-protein-lipid-bilayer-complex/gromacs, before you run the command
        ...
        st.write("✔️create an index group for TM helices and three ECL residues (residue 188, 190 & 194) only (since they are ligand binding residues), followed by extracting the topology information and MD simulation trajectory of TM helices and three ECL residues (residue 188, 190 & 194) only. Expect to observe lower RMSD values since loops no longer dominate the motion, termini no longer inflate RMSD, and only the GPCR core is measured")
        st.code("""
        gmx make_ndx -f protein_ligand_complex.tpr -o tm_helices_ligand.ndx # create the index for both TM helices of the protein and its ligand only. Type, r 31-54 | r 66-87 | r 104-125 | r 146-167 | r 188 | r 190 | r 194 | r 195-215 | r 279-298 | r 311-334, and press "Enter", followed by typing, name 10 TM_ecl_helices, to rename the newly created index group as TM_ecl_helices and position the newly created index group as 10. Type "q" to quit
        gmx convert-tpr -s protein_ligand_complex.tpr -n tm_helices_ligand.ndx -o tm_helices_ligand.tpr # Select 10 (TM_ecl_helices) to extract TM_ecl-only topology
        gmx trjconv -s protein_ligand_complex.tpr -f protein_ligand_complex_fit.xtc -n tm_helices_ligand.ndx -o tm_helices_ligand.xtc # Select 10 (TM_ecl_helices) to extract TM_ecl-only trajectory
        """, language="bash")
