...
INDEX_PREFIX="/media/cbr14/Two/Wee/WeeYeZhi/output/bowtie2_results/bowtie2_build_results/trinity_assembly_after_cdhitest_trinity_NCBI_FCS_transrate/trinity_assembly_after_cdhitest_trinity_NCBI_FCS_transrate"
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    OUTDIR="/media/cbr14/Two/Wee/WeeYeZhi/output/bowtie2_results/bowtie2_mapping_results/trinity_assembly_after_cdhitest_trinity_NCBI_FCS_transrate/${SAMPLE}"
    OUTBAM="${OUTDIR}/${SAMPLE}.bam"
    ALIGN_STATS="${OUTDIR}/align_stats.txt"

    # Create output directory if it does not exist
    mkdir -p "${OUTDIR}"

    echo "Running bowtie2 for sample ${SAMPLE}..."
    bowtie2 -p ${THREADS} -q --no-unal -k 20 -x "${INDEX_PREFIX}" -1 "${READ1}" -2 "${READ2}" 2> "${ALIGN_STATS}" | samtools view -@ 16 -Sb -o "${OUTBAM}"
